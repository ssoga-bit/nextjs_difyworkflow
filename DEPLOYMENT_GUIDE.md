...
現在、このアプリは**51分のワークフロー**を実行できますが、Vercelには以下の制限があります：
...
長時間ワークフローには、以下のいずれかを使用：

1. **Vercel Edge Functions** + **Queue システム**（例: Upstash QStash）
2. **外部ワーカー**（例: Railway, Render, Fly.io）
